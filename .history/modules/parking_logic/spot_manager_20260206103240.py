...
class SpotManager:
    ...
    def check_occupancy(self, cars):
        """Standard IoU check between Cars and Spots."""
        spot_statuses = []
        ...
        for car in cars:
            ...
            for idx, status in enumerate(spot_statuses):
                spot_box = status["bbox"]

                # Check if car center is inside spot (Fast check)
                if (spot_box[0] < cx < spot_box[2]) and (
                    spot_box[1] < cy < spot_box[3]
                ):
            ...
            # If match found
            if best_spot_idx != -1 and best_iou > 0.10:  # Low threshold
                spot_statuses[best_spot_idx]["occupied"] = True
                spot_statuses[best_spot_idx]["car_id"] = car["id"]
